...
- **Repos always resolve against live CRAN, not a base image's frozen snapshot.** Many R base images (`rocker/geospatial` included) point the default `"CRAN"` repo at a Posit Package Manager snapshot frozen on the date the image was built. `install_deps.R` overrides this with `options(repos = c(CRAN = "https://cloud.r-project.org"))` before installing anything, so a `renv.lock`-pinned package version released *after* that image's snapshot date can still be fetched — otherwise `renv::restore()` would 404 indefinitely trying to find it in a mirror that stopped moving forward on the image's build date.
