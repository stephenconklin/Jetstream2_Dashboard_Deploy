...
**To swap in a completely different app on the same instance:** point `build_and_run.sh` at the new project instead — e.g. `./deploy/build_and_run.sh /path/to/other-project` — keeping the *same* image/container name each time (the default, `shiny-app`, when you don't pass one). `run_container()` in `common.sh` runs `docker rm -f <container-name>` before starting the new one, so this cleanly stops and removes whatever's currently running under that name and replaces it — no manual cleanup needed, and the old image tag just gets overwritten rather than accumulating on disk.

This only works cleanly if the name stays consistent across deploys. This tool is scoped to one app at a time per instance (see "Assumes one instance per project/researcher" at the top of this doc) — it doesn't support two apps running simultaneously: every container binds host port 80, so a *second*, differently-named container started while the first is still running will fail with "port is already allocated," and using a *different* name means the old container is never torn down (it just keeps squatting on port 80). If you want two dashboards reachable at once, provision a second Jetstream2 instance rather than trying to run both here.
