...
- **Redeploying replaces the running container in place, keyed off container name.** `run_container()` runs `docker rm -f "$CONTAINER_NAME"` before every `docker run`, so re-running `build_and_run.sh` against a *different* project — as long as it uses the same (default `shiny-app`, or an explicit) image-name argument — cleanly tears down whatever's currently running and swaps in the new one. This only works if the name stays consistent: since every container binds host port 80 unconditionally, a differently-named second deploy won't replace the first — it'll just fail to start ("port is already allocated"), because this tool is scoped to one running app per instance at a time (see "Design constraints" below), not multiple apps side by side.
